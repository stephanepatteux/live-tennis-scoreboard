"""Break-point / score alerts for the trader board.

Alerts are computed here (not taken from the data provider) so the board's
strategy filters — 0-40, 15-40, 30-40, any break point, deuce, tiebreak — mean
exactly the same thing whether the feed is live or demo. An alert only fires when
the *returner* (non-server) is ahead in the game, because that is the situation a
Betfair trader reacts to: the serve is under threat.
...
# Higher priority = hotter alert. Used for the "Alerts first" sort and the
# hot-card styling on the board (>= 4 renders as a hot card).
CODE_PRIORITY = {
    "0-40": 6,
    "ad-break": 5,
    "15-40": 4,
...
# Human-readable label for the toast + in-card cue (highest-priority alert wins).
CODE_LABEL = {
...
def compute_alerts(points_p1, points_p2, server, is_tiebreak):
    ...
    are out of scope for this board).
    """
    alerts: list[str] = []
