"""In-memory pub/sub hub between a push source and the browser SSE clients.

A single source thread calls :meth:`publish` on every score commit; each connected
browser has a queue that receives the updated match. The hub also keeps the latest
state per match so a newly-connected browser gets an immediate snapshot.
"""
    ...
    def publish(self, match: dict) -> None:
        ...
        for q in subs:
            try:
                ...
                pass  # slow client; it will catch up on its next state anyway
