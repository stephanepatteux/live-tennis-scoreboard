"""Map a Live Tennis API match/score frame into the board's match shape.

Used for the Ultra WebSocket push frames. Scores are PLAYER-MAJOR:
``sets=[p1,p2]``, ``games=[[p1 per set],[p2 per set]]``, ``points=[p1,p2]``,
``server`` is 1 or 2. Reference: https://docs.livetennisapi.com (OpenAPI schema).
"""
...
def map_api_match(frame: dict, model_fallback: bool = True, static=None) -> dict:
    """Map one Match/score frame to a normalised board match.
